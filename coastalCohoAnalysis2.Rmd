...
```{r eval=params$runModels, warning=FALSE}
...
```{r eval=!params$runModel}
m1 <- readRDS("m1.Rdat")
m2 <- readRDS("m2.Rdat")
m3 <- readRDS("m3.Rdat")
```

We place constraints on productivity using data from Bradford 1995. Average egg to smolt survival ranged from `r round(qM[1],1)`% to `r round(qM[5],1)`% for the `r length(sDat$mean)` populations with a mean and standard deviation of `r round(mean(sDat$mean),1)`% and `r round(mean(sDat$mean),1)`%. Assuming an average fecundity of 3,000, the range of expected smolts per spawner is 0.5 $\cdot$ 3000 $\cdot$ (`r round(qM[1],1)`, `r round(qM[5],1)`) = (`r round(0.5*3000*qM[1],1)`,`r round(0.5*3000*qM[3],1)`) with a mean and standard deviation of `r round(mean(0.5*3000*sDat$mean),1)` and `r round(sd(0.5*3000*sDat$mean),1)`
...
-  We can talk about the potential for improvements assuming different variables. For example, we can't use intrinsic potential since it doesn't depend on habitat restoration (I think, right?). But pool area could be used. 
...
-  Should be be treating the basins above the traps as populations? Is there likely a lot of straying between these sub-populations and other locations in the basins. What are the alternatives? limiting to adult to smolt? 
...
for(st in 1:bdat$Npops){
  ...
  lines(xx,srF(exp(r1$estimate),ss=xx),lwd=2,col="black")
