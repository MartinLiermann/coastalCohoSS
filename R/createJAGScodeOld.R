...
model
{
  ...
  for(i in 1:N){ # iterate over all years and populations
    ...
    logit(oceanSurv[i]) <- oceanSurvL[i]
  ...
  ### year effects (SR resids), smolt residuals common to all populations
  for(k in 1:Nyears){
  ...
  for(i in 1:NescFem){
   escapementFemObs[i] ~ dlnorm(log(spawnersFemWild[escFemInd[i]]),escFemObsTau)
  }
  escObsTau ~ dgamma(0.001,0.001)
  escFemObsTau ~ dgamma(0.001,0.001)
