...
####################################################
createInitValsFunc3 <- function(dat, includePlots=FALSE){
  bdat <- dat$jagsDat
  ...
  function(){
    ...
    smDat <- if("smoltDat" %in% names(bdat))
    ...
    if(smDat) smolt <- edat$smoltObs
    else smolt <- c((edat$escapementObs/edat$HR/edat$OS)[-3],rep(NA,3))
    habVar <- bdat$habVar
